...
ss.results.BART <- mclapply(
  ...
  function(x) PM_SimStudy_BART(
    ...
    burnin = 5000,
    param_ls = c(0.75, 1.5, -0.5, 0.05, 4, 2, -1, 0.1, -0.25, -1.5, -0.5, -0.75),    
    model = 2
  ),
  mc.cores = workers
)
...
# save result
saveRDS(ss.results.BART, file = here::here("output", "simstudy-pm2-bart.RDS"))
